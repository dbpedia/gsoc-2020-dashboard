...
TOTAL_TRIPLES = "SELECT (COUNT(*) AS ?x) WHERE { ?s ?p ?o }"

TOTAL_CLASSES = "SELECT (COUNT(DISTINCT ?o) AS ?x) WHERE { ?s a ?o }"

TOTAL_PROPERTIES = "SELECT (COUNT(DISTINCT ?p) AS ?x) WHERE { ?s ?p ?o }"

BLANK_SUBJECTS = "SELECT (COUNT(DISTINCT ?s) AS ?x) WHERE { ?s ?p ?o FILTER(isBlank(?s))}"

BLANK_OBJECTS = "SELECT (COUNT(DISTINCT ?o ) AS ?x) WHERE { ?s ?p ?o FILTER(isBlank(?o))}"
